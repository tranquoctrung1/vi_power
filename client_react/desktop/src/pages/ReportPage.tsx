...
export default function ReportPage() {
  ...
  const allAreasLabel =
    !storeUser || storeUser.role === "Admin"
      ? "Toàn nhà máy"
      : "Tất cả khu vực";
  ...
  async function loadMeta() {
    try {
      ...
      showToast("Lỗi tải dữ liệu thiết bị/khu vực" + err, "error");
    }
  }
  ...
  function getTargets(): Device[] {
    if (selDevice !== "all")
      return devices.filter((d) => d.deviceid === selDevice);
    if (selArea !== "all")
      return devices.filter((d) => d.displaygroupid === selArea);
    return devices;
  }
  ...
  async function loadReport() {
    ...
    const { start, end, prevStart, prevEnd } = computePeriod(
      range,
      dateFrom,
      dateTo,
    );
    ...
    try {
      ...
      const cBuckets: Record<string, { p: number; n: number }>[] = Array.from(
        { length: bucketCount },
        () => ({}),
      );
      ...
      currRes.forEach(({ device, rows }) => {
        ...
        cFirstNet[device.deviceid] = rows.length
          ? (rows[0].netpower ?? null)
          : null;
        rows.forEach((row) => {
          const bi = Math.floor(
            (new Date(row.timestamp).getTime() - start.getTime()) / bucketMs,
          );
          if (bi >= 0 && bi < bucketCount) {
            const b =
              cBuckets[bi][device.deviceid] ||
              (cBuckets[bi][device.deviceid] = { p: 0, n: 0 });
            b.p += row.power || 0;
            b.n++;
          ...
          tableData.push({
            ...
            area: getGroupName(device.displaygroupid),
            areaId: device.displaygroupid || "",
            power: row.power || 0,
            kwh: (row.power || 0) * ((device.samplingCycle || 60) / 3600),
            pf: row.per ?? null,
            status: (row.power || 0) > 0 ? "ok" : "error",
            i1: row.currentI1 ?? null,
            i2: row.currentI2 ?? null,
            i3: row.currentI3 ?? null,
            v1n: row.voltageV1N ?? null,
            v2n: row.voltageV2N ?? null,
            v3n: row.voltageV3N ?? null,
            kw1: row.kw1 ?? null,
            kw2: row.kw2 ?? null,
            kw3: row.kw3 ?? null,
            totalKva: row.totalKva ?? null,
            totalKvar: row.totalKvar ?? null,
            freq: row.freq ?? null,
            kvah: row.kvah ?? null,
            kvarh: row.kvarh ?? null,
          });
        });
      ...
      const avgPow = cBuckets.map((b) =>
        Object.values(b).reduce((s, d) => s + (d.n ? d.p / d.n : 0), 0),
      );
      ...
      const energyBucketsFromNetpower = (
        ...
        devs.forEach((dev) => {
          ...
          for (let bi = 0; bi < bucketCount; bi++) {
            ...
            if (lastVal != null)
              buckets[bi] += Math.max(0, netByIdx[bi] - lastVal);
            lastVal = netByIdx[bi];
          }
        });
        return buckets;
      ...
      const kwhBuckets = energyBucketsFromNetpower(
        cNetByIdx,
        cFirstNet,
        targets,
      );
      ...
      const pBuckets: Record<string, { p: number; n: number }>[] = Array.from(
        { length: bucketCount },
        () => ({}),
      );
      ...
      prevRes.forEach(({ device, rows }) => {
        const netByIdx: Record<number, number> = {};
        pFirstNet[device.deviceid] = rows.length
          ? (rows[0].netpower ?? null)
          : null;
        rows.forEach((row) => {
          const bi = Math.floor(
            (new Date(row.timestamp).getTime() - prevStart.getTime()) /
              bucketMs,
          );
          if (bi >= 0 && bi < bucketCount) {
            const b =
              pBuckets[bi][device.deviceid] ||
              (pBuckets[bi][device.deviceid] = { p: 0, n: 0 });
            b.p += row.power || 0;
            b.n++;
            if (row.netpower != null) netByIdx[bi] = row.netpower;
          }
      ...
      const prevAvgPow = pBuckets.map((b) =>
        Object.values(b).reduce((s, d) => s + (d.n ? d.p / d.n : 0), 0),
      );
      const prevKwhBuckets = energyBucketsFromNetpower(
        pNetByIdx,
        pFirstNet,
        targets,
      );
      ...
      setCurrKwh(totalKwh);
      setPrevKwh(prevTotalKwh);
      setCurrPeak(peakCurr);
      setPrevPeak(peakPrev);
      setAvgPf(
        pfPerDevice.length
          ? pfPerDevice.reduce((a, b) => a + b, 0) / pfPerDevice.length
          : null,
      );
    ...
    powerInst.current = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: "Kỳ trước (kW)",
            data: prev.map((v) => +v.toFixed(2)),
            borderColor: "rgba(56,170,255,0.3)",
            borderWidth: 1.5,
            pointRadius: 0,
            tension: 0.4,
          ...
          {
            label: `Ngưỡng ${THRESHOLD_KW} kW`,
            data: Array(labels.length).fill(THRESHOLD_KW),
            borderColor: "rgba(244,75,75,0.6)",
            borderWidth: 1.5,
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            borderColor: "rgba(56,139,253,.28)",
            borderWidth: 1,
            titleColor: "#607b99",
            bodyColor: "#e6eef8",
            padding: 10,
        ...
        scales: {
          x: {
            grid: { color: "rgba(56,139,253,.05)" },
            ticks: { color: "#3a506b", font: { size: 10 }, maxTicksLimit: 12 },
          },
          y: {
            grid: { color: "rgba(56,139,253,.07)" },
            ticks: { color: "#607b99", font: { size: 10 } },
            beginAtZero: true,
          },
        },
      },
          ...
          tooltip: {
            ...
            callbacks: {
              label: (c: {
                ...
                ` ${c.dataset.label}: ${Number(c.parsed.y ?? 0).toLocaleString("vi-VN", { maximumFractionDigits: 2 })} kWh`,
            },
          },
        },
        scales: {
  ...
  function buildCumChart(labels: string[], curr: number[], prev: number[]) {
    const canvas = cumRef.current;
    if (!canvas) return;
    let cc = 0,
      cp = 0;
    const cumCurr = curr.map((v) => {
      cc += v;
      return +cc.toFixed(2);
    });
    const cumPrev = prev.map((v) => {
      cp += v;
      return +cp.toFixed(2);
    });
    cumInst.current?.destroy();
        ...
        datasets: [
          {
            ...
            data: cumCurr,
            borderColor: "#22d369",
            backgroundColor: "rgba(34,211,105,0.08)",
            borderWidth: 2,
            pointRadius: 0,
            tension: 0.35,
            fill: true,
          ...
          tooltip: {
            callbacks: {
              label: (c: {
                ...
                ` ${c.dataset.label}: ${Number(c.parsed.y ?? 0).toLocaleString("vi-VN", { maximumFractionDigits: 1 })} kWh`,
            },
          },
        },
        scales: {
          x: {
            ticks: { color: "#607b99", font: { size: 9 }, maxTicksLimit: 10 },
            grid: { display: false },
          },
          y: {
            ticks: { color: "#607b99", font: { size: 10 } },
            grid: { color: "rgba(56,139,253,0.08)" },
            beginAtZero: true,
          },
        },
      },
    ...
    results.forEach(({ device, rows }) => {
      rows.forEach((row) => {
        ...
        if (day >= 0 && day < 7) {
          ...
          const b =
            cell[device.deviceid] || (cell[device.deviceid] = { p: 0, n: 0 });
          b.p += row.power || 0;
          b.n++;
        }
      });
    });
    const avg = cells.map((row) =>
      row.map((cell) =>
  ...
  async function loadBomReport() {
    ...
    setBomLoading(true);
    try {
      const start = new Date(bomDate);
      start.setHours(0, 0, 0, 0);
      const end = new Date(bomDate);
      ...
      interface AggHourRow {
        _id: string;
        avgVoltageV1N?: number;
        avgVoltageV2N?: number;
        avgVoltageV3N?: number;
        avgCurrentI1?: number;
        avgCurrentI2?: number;
        avgCurrentI3?: number;
        avgPower?: number;
  ...
  async function handleBomExport() {
    ...
    await exportBomReport(
      device?.deviceName || effectiveBomDeviceId,
      bomDate,
      bomRows,
    );
  ...
  const tableFiltered = tableRows.filter((r) => {
    ...
    return (
      r.device.toLowerCase().includes(q) || r.area.toLowerCase().includes(q)
    );
  });
  ...
  const tableSorted = [...tableFiltered].sort((a, b) => {
    ...
    if (sortCol === "time") {
      ...
      va = (a[sortCol as keyof typeof a] as number) ?? -1;
      vb = (b[sortCol as keyof typeof b] as number) ?? -1;
    }
  ...
  const tablePaged = tableSorted.slice(
    (tablePage - 1) * PAGE_SIZE,
    tablePage * PAGE_SIZE,
  );
  const tableAvgPow = tableFiltered.length
    ? tableFiltered.reduce((a, r) => a + r.power, 0) / tableFiltered.length
    : 0;
  ...
  const tableTotalKwh = tableSearch
    ? tableFiltered.reduce((a, r) => a + r.kwh, 0)
    : (currKwh ?? 0);
  ...
  tableFiltered.forEach((r) => {
    if (r.pf !== null && (r.pf ?? 0) > 0)
      (pfByDevice[r.device] ||= []).push(r.pf as number);
  });
  const pfDeviceAvgs = Object.values(pfByDevice).map(
    (arr) => arr.reduce((a, b) => a + b, 0) / arr.length,
  );
  const tableAvgPf = pfDeviceAvgs.length
    ? pfDeviceAvgs.reduce((a, b) => a + b, 0) / pfDeviceAvgs.length
    : null;

  const areaColorMap: Record<string, string> = {};
  groups.forEach((g, i) => {
    areaColorMap[g.displaygrouid || g._id] =
  ...
  return (
    <Layout
      ...
      breadcrumb={["ViPower", "Giám sát", "Báo cáo"]}
    ...
    >
      ...
      <div className="filter-bar">
        ...
        <div className="range-pills">
          {RANGE_OPTS.map((o) => (
            <button
              key={o.id}
              className={`range-pill${range === o.id ? " active" : ""}`}
      ...
      >
        <div
          className="kpi-card"
          style={{ ["--kpi-color" as string]: "var(--accent)" }}
        >
          <div className="kpi-label">Tổng điện năng</div>
          <div className="kpi-value">
            {fmt(currKwh, 0)} <sup>kWh</sup>
          </div>
          <div className="kpi-footer">
            <div className={`kpi-delta ${energyDelta.cls}`}>
              {energyDelta.text}
            </div>
        ...
        <div
          className="kpi-card"
          style={{ ["--kpi-color" as string]: "var(--yellow)" }}
        >
          <div className="kpi-label">Công suất đỉnh</div>
          <div className="kpi-value">
            {fmt(currPeak, 0)} <sup>kW</sup>
          </div>
          <div className="kpi-footer">
        ...
        <div
          className="kpi-card"
          style={{ ["--kpi-color" as string]: "var(--green)" }}
        >
          <div className="kpi-label">Hệ số công suất TB</div>
          <div className="kpi-value">
            {avgPf !== null ? fmt(avgPf * 100, 1) : "--"} <sup>%</sup>
          </div>
          <div className="kpi-footer">
            <div className="kpi-delta" style={{ color: "var(--text-dim)" }}>
              {avgPf !== null ? "" : "N/A"}
            </div>
        ...
        <div className="card">
          <div className="card-header">
            <span className="card-title">
              ...
              Công suất theo thời gian (kW)
            </span>
            <span style={{ fontSize: 11, color: "var(--text-dim)" }}>
              {selArea === "all" ? allAreasLabel : getGroupName(selArea)}
            </span>
          </div>
      ...
      <div className="section-row">
        <span className="section-label">
          Heatmap công suất — 7 ngày gần nhất (kW)
        </span>
        <div className="section-line"></div>
        <span style={{ fontSize: 11, color: "var(--text-dim)" }}>
        ...
        <div className="heatmap-wrap">
          <div className="heatmap-grid">
            ...
            {Array.from({ length: 24 }, (_, h) => (
              <div key={h} className="hm-header">
                {h % 3 === 0 ? `${String(h).padStart(2, "0")}h` : ""}
              </div>
            ))}
            {hmDays.flatMap((day, di) => [
              <div key={`l${di}`} className="hm-label">
                {day}
              </div>,
          ...
          <div className="hm-legend">
            ...
            <div className="hm-legend-bar">
              {Array.from({ length: 10 }, (_, i) => (
                <div
                  key={i}
                  className="hm-legend-seg"
                  style={{ background: heatColor((i / 9) * heatMax, heatMax) }}
                />
              ))}
            </div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-header">
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <select
              className="filter-select"
              value={effectiveBomDeviceId}
              onChange={(e) => setBomDeviceId(e.target.value)}
            >
              {devices.map((d) => (
                <option key={d.deviceid} value={d.deviceid}>
                  {d.deviceName || d.deviceid}
          ...
          <div className="table-scroll">
            <table className="data-table">
              <thead>
                <tr>
                  <th>Giờ</th>
                  <th className="num">Uab (V)</th>
                  <th className="num">Ubc (V)</th>
                  <th className="num">Uca (V)</th>
                  <th className="num">Utb (V)</th>
                  <th className="num">Itb (A)</th>
                  <th className="num">Ia (A)</th>
                  <th className="num">Ib (A)</th>
                  <th className="num">Ic (A)</th>
                  <th className="num">P (KW)</th>
                </tr>
              </thead>
              <tbody>
                {bomRows.map((r) => (
                  <tr key={r.hour}>
                    <td>{r.hour}</td>
                    <td className="num">{fmt(r.uab, 1)}</td>
                    <td className="num">{fmt(r.ubc, 1)}</td>
                    <td className="num">{fmt(r.uca, 1)}</td>
                    <td className="num">{fmt(r.utb, 1)}</td>
                    <td className="num">{fmt(r.itb, 2)}</td>
                    <td className="num">{fmt(r.ia, 2)}</td>
                    <td className="num">{fmt(r.ib, 2)}</td>
                    <td className="num">{fmt(r.ic, 2)}</td>
                    <td className="num">{fmt(r.p, 2)}</td>
                  </tr>
                ))}
              </tbody>
              {bomRows.length > 0 && (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div className="search-box">
              ...
              <input
                ...
                type="search"
                placeholder="Tìm thiết bị, khu vực..."
                value={tableSearch}
                onChange={(e) => {
                    ...
                    {(
                      ...
                      return (
                        <th
                          key={col}
                          data-col={col}
                          className={`sortable${num ? " num" : ""}${active ? " sort-" + sortDir : ""}`}
                  ...
                  {tablePaged.map((r, i) => {
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td className="num">
                          {r.pf != null ? fmt(r.pf * 100, 1) : "--"}
                        </td>
                        <td>
                          <span className={`status-badge ${stCls}`}>
                            {stLabel}
                          </span>
                        </td>
                      </tr>
