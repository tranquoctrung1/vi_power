...
const EnergyDataModel = {
    ...
    async create(deviceid, energyData) {
        try {
    ...
    async createMany(deviceid, energyDataArray) {
        try {
            const collectionName = this.getCollectionName(deviceid);
            const collection = await this.getCollection(collectionName);
    ...
    async findByTimeRange(deviceid, startTime, endTime, options = {}) {
        try {
            const collectionName = this.getCollectionName(deviceid);
            const collection = await this.getCollection(collectionName);
    ...
    async findLatest(deviceid, limit = 1) {
        try {
            const collectionName = this.getCollectionName(deviceid);
            const collection = await this.getCollection(collectionName);
    ...
    // READ - Lấy thống kê
    async getStats(deviceid, startTime, endTime) {
        try {
            const collectionName = this.getCollectionName(deviceid);
            const collection = await this.getCollection(collectionName);
    ...
    async deleteOldData(deviceid, olderThanDays = 30) {
        try {
            const collectionName = this.getCollectionName(deviceid);
            const collection = await this.getCollection(collectionName);
