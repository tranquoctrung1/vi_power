...
const DeviceModel = {
  ...
  async create(db, deviceData) {
    try {
      ...
      await DeviceModel._createEnergyDataCollection(db, deviceData.deviceid);
  ...
  async _createEnergyDataCollection(db, deviceid) {
    try {
      ...
      if (collections.length === 0) {
        ...
        console.log(`✅ Created energy_data collection: ${collectionName}`);

        // Tạo index cho timestamp (thứ tự tăng dần: 1)
        await db.collection(collectionName).createIndex({ timestamp: 1 });
        console.log(`✅ Created timestamp index (1) for ${collectionName}`);
  ...
  async _deleteEnergyDataCollection(db, deviceid) {
    try {
      ...
      if (collections.length > 0) {
        ...
        console.log(`✅ Deleted energy_data collection: ${collectionName}`);
      }
    } catch (error) {
      console.error(
    ...
    try {
      ...
      const stats = {
        ...
        byStatus: {
          ...
          inactive: await devices.countDocuments({ status: "inactive" }),
      ...
      const typeStats = await devices
        .aggregate([{ $group: { _id: "$deviceType", count: { $sum: 1 } } }])
        .toArray();
