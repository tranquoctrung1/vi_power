...
const userGroupController = {
  ...
  async addUserToGroup(req, res) {
    try {
      ...
      const user = await UserModel.findByUsername(db, userGroupData.username);
      if (!user) {
  ...
  async getUsersInGroup(req, res) {
    try {
      ...
      // Kiểm tra group tồn tại
      const group = await DisplayGroupModel.findByGroupId(db, displaygrouid);
      if (!group) {
        return res.status(404).json({
          success: false,
          message: "Display group not found",
      ...
      const users = await UserGroupModel.findUsersInGroupWithDetails(
        db,
        displaygrouid,
      );
  ...
  async getUserGroups(req, res) {
    try {
      ...
      const userGroups = await UserGroupModel.findByUsername(db, username);
  ...
  async checkUserInGroup(req, res) {
    try {
      ...
      res.json({
        success: true,
        isInGroup,
        message: isInGroup ? "User is in group" : "User is not in group",
      });
    } catch (error) {
      res.status(500).json({
        success: false,
      ...
      res.json({
        ...
        stats: {
          ...
          averageUsersPerGroup:
            totalDisplayGroups > 0 ? totalUserGroups / totalDisplayGroups : 0,
          groupStats,
          userStats,
        },
      });
    ...
    try {
      ...
      if (!displaygrouid || !Array.isArray(usernames)) {
        return res.status(400).json({
          success: false,
          message: "Display group ID and usernames array are required",
        });
      }
      ...
      for (const username of usernames) {
        try {
          ...
          const user = await UserModel.findByUsername(db, username);
          if (!user) {
            results.failed.push({ username, error: "User not found" });
            continue;
          }
          ...
          // Thêm vào group
          await UserGroupModel.create(db, { displaygrouid, username });
